...
class MonoResConv(nn.Module):
    """
    Predict depth residual from feature + mono prior.
    Low-intrusive replacement of Conv3x3(out_ch, 1).
    """

    def __init__(self, in_ch, hidden_ch=None, norm_layer=nn.BatchNorm2d, act=nn.ReLU):
        super().__init__()
        if hidden_ch is None:
            hidden_ch = in_ch

        # project mono depth into feature space
        self.mono_proj = nn.Sequential(
            Basic2d(1, hidden_ch, norm_layer=norm_layer, act=act, kernel_size=3, padding=1),
            Basic2d(hidden_ch, hidden_ch, norm_layer=norm_layer, act=act, kernel_size=3, padding=1),
        )

        # fuse feature + mono feature
        self.fuse = nn.Sequential(
            Basic2d(in_ch + hidden_ch, in_ch, norm_layer=norm_layer, act=act, kernel_size=3, padding=1),
            Basic2d(in_ch, in_ch, norm_layer=norm_layer, act=act, kernel_size=3, padding=1),
        )

        # predict residual
        # self.out = Conv3x3(in_ch, 1, bias=True)

    def forward(self, fout, mono):
        """
        fout: B x C x H x W
        mono: B x 1 x H x W
        """
        mono_feat = self.mono_proj(mono)
        x = torch.cat([fout, mono_feat], dim=1)
        x = self.fuse(x)
        return x
        # res = self.out(x)
        return res
    
def mono_ordinal_prior(mono, k=31, eps=1e-6):
    # mono: [B,1,H,W], larger depth = farther
    inv = 1.0 / mono.clamp(min=eps)   # closer -> larger

    mean = F.avg_pool2d(inv, k, stride=1, padding=k // 2)
    mean2 = F.avg_pool2d(inv * inv, k, stride=1, padding=k // 2)
    std = (mean2 - mean * mean).clamp(min=eps).sqrt()

    return (inv - mean) / std

class PMP_residual(nn.Module):
    """
    Pre + MF + Post with mono-guided residual update.
    Mono depth only participates in MF residual prediction:
        res = conv(fout, mono)
    """
    ...
    def __init__(self, level, in_ch, out_ch, drop_path, up=True, pool=True):
        ...
        # new mono-guided residual head
        self.conv_mono = MonoResConv(out_ch)
    ...
    def forward(self, fout, dout, XI, S, K, metric_aligned, normalize_mono=False):
        # S: Sparse depth map
        # K: camera

        ###############################################################
        # feature upsample / merge
        fout = self.upcat(XI, fout, dout)

        ###############################################################
        # sparse anchor (keep original PMP behavior)
        Sp = self.wpool(S, fout)

        # mono prior at current pyramid level
        metric_p = F.interpolate(
            metric_aligned, size=Sp.shape[-2:], mode="bilinear", align_corners=False
        )
        ...
        xx, yy = torch.meshgrid(
            torch.arange(width, device=Sp.device),
            torch.arange(height, device=Sp.device),
            indexing='xy'
        )
        ...
        ###############################################################
        # MF
        # use current depth estimate to update geometry-aware feature
        Pxyz = self.pinv(dout, Kp, xx, yy).view(dout.shape[0], 3, dout.shape[2], dout.shape[3])
        fout = self.fuse(fout, Pxyz)

        # mono-guided residual correction
        if normalize_mono:
            # print("[debug] normalize_mono:", normalize_mono)
            mono_prior = mono_ordinal_prior(metric_p)
            res = self.conv_mono(fout, mono_prior)
        else:
            res = self.conv_mono(fout, metric_p)
        res = self.conv(res)
        dout = dout + res
...
class CSPN_mono(nn.Module):
    ...
    def __init__(self, in_channels, pt, norm_layer=nn.BatchNorm2d, act=nn.ReLU, eps=1e-6):
        ...
        self.convct = nn.Sequential(
            Basic2d(in_channels + 4, in_channels, norm_layer=norm_layer, act=act),
            Basic2d(in_channels, 3, norm_layer=None, act=functools.partial(nn.Softmax, dim=1)),
        )

    @custom_fwd(cast_inputs=torch.float32)
    def forward(self, fout, hn, h0, mono):
        weight3x3 = self.weight3x3(fout)
        weight5x5 = self.weight5x5(fout)
        weight7x7 = self.weight7x7(fout)
        mask3x3, mask5x5, mask7x7 = torch.split(self.convmask(fout) * (h0 > 1e-3).float(), 1, dim=1)
        ...
        hns.append(conf3x3 * hn3x3 + conf5x5 * hn5x5 + conf7x7 * hn7x7)
        hns = torch.cat(hns, dim=1)
        wt = self.convct(torch.cat([fout, hns, mono], dim=1))
        hn = torch.sum(wt * hns, dim=1, keepdim=True)
        return hn
    
class PMP_mono(nn.Module):
    """
    Pre + MF + Post with mono-guided residual update.
    Mono depth only participates in MF residual prediction:
        res = conv(fout, mono)
    ...
    def __init__(self, level, in_ch, out_ch, drop_path, up=True, pool=True):
        ...
        self.cspn = CSPN_mono(out_ch, pt=2 * (6 - level))
    ...
    def forward(self, fout, dout, XI, S, K, metric_aligned, normalize_mono=False):
        ...
        if normalize_mono:
            # print("[debug] normalize_mono:", normalize_mono)
            metric_p = mono_ordinal_prior(metric_p)
        res = self.conv(fout)
        dout = dout + res

        ###############################################################
        # Post (unchanged)
        dout = self.cspn(fout, dout, Sp, metric_p)
