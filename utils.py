...
def load_state_dict_filter_shape(model, state_dict, verbose=True):
    ...
    for k, v in state_dict.items():
        if k not in model_dict:
            skipped.append((k, tuple(v.shape), None, "not in current model"))
            continue

        if model_dict[k].shape != v.shape:
            skipped.append((k, tuple(v.shape), tuple(model_dict[k].shape), "shape mismatch"))
            continue

        matched[k] = v

    if verbose:
        print(f"[load ckpt] matched params: {len(matched)}")
        print(f"[load ckpt] skipped params: {len(skipped)}")

        for k, ckpt_shape, model_shape, reason in skipped:
            print(f"[skip] {k}: ckpt={ckpt_shape}, model={model_shape}, reason={reason}")

    model_dict.update(matched)
    msg = model.load_state_dict(model_dict, strict=False)

    if verbose:
        print("[load ckpt] missing keys:", msg.missing_keys)
        print("[load ckpt] unexpected keys:", msg.unexpected_keys)
...
class Trainer(object):
    ...
    def init_net(self):
        ...
        if 'chpt' in self.cfg:
            ...
            if os.path.isfile(self.cfg.chpt):
                ckpt_path = self.cfg.chpt
            else:
                save_path = os.path.join('checkpoints', self.cfg.chpt)
                ckpt_path = os.path.join(save_path, 'result_ema.pth')
            print("[debug] loading:", ckpt_path)
            cp = torch.load(ckpt_path, map_location=torch.device('cpu'))
            # model.load_state_dict(cp['net'], strict=False)
            load_state_dict_filter_shape(model, cp['net'], verbose=True)
    ...
    def save_state(self, name="result_ema"):
        if self.rank:
            return
        ...
        print("[save to]:", os.path.join(save_path, f'{name}.pth'))
        torch.save(state_dict, os.path.join(save_path, f'{name}.pth'))
