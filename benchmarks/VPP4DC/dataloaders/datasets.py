...
class UNIDataset(data.Dataset):
    def __init__(self, datapath, test=True, overfit=False, baseline=None, topcrop = 100):
        self.topcrop = topcrop

        self.is_test = test
        self.init_seed = False
        self.baseline = baseline
        self.files = sorted(glob(osp.join(datapath, '*.npy')))
    ...
    def __getitem__(self, index):
        ...
        hints_depth, gt_depth, K, rgb, filedir = self.__getraw__(index)
        hints_depth = hints_depth[:, :, None]
        ...
        #Read depth 
        data['validgt'] = (gt_depth > 0.0).astype(np.uint8)
        data['validhints'] = (hints_depth > 0.0).astype(np.uint8)
        ...
        baseline = 0.53275 if self.baseline is None else self.baseline
        fhints_depth, _ = np.expand_dims(filter_heuristic_depth(hints_depth[..., 0], nx=7, ny=7, th=1.5), -1)
        fvalidhints_depth = np.where(fhints_depth > 0, 1, 0)
        data['fvalidhints'] = np.logical_and(fvalidhints_depth, data['validhints']).astype(np.uint8)
...
def fetch_dataloader(args):
    ...
    if args.dataset == 'kittidcval':
        if args.test:
            ...
            dataset = UNIDataset(args.datapath,test=True,baseline=args.baseline)
            loader = data.DataLoader(dataset, batch_size=args.batch_size, 
                pin_memory=False, shuffle=False, num_workers=0, drop_last=True)
            print(f'Testing using {args.dataset} with {len(loader.dataset)} image pairs')
