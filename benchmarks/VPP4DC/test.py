...
def main():
    ...
    acc_list = []
    run.counter = 0
    os.makedirs(args.vis_dir, exist_ok=False)
    ...
    for asd in range(args.tries):
        ...
        for batch_idx, datablob in enumerate(demo_loader):
            ...
            if True:
                pred_np = result['pred'].squeeze().detach().cpu().numpy().astype(np.float32)
                ...
                cv2.imwrite(os.path.join(args.vis_dir ,f"{batch_idx:08d}_depth.png"), pred_vis)
